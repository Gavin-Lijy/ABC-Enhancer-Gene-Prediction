...
def get_model_argument_parser():
    ...
    parser.add_argument('--promoter_activity_quantile_cutoff', type=float, default=0, help="Quantile cutoff on promoter activity. Used to consider a gene 'expressed' in the absence of expression data")
...
def main():
    ...
    genelist_slim_expressed_to_combine = os.path.join(args.outdir, "GeneListExpressed_slim_to_combine.txt")
    print(genes.head(n=10))
    genes.to_csv(genelist_slim, sep="\t", index=False, header=True, float_format="%.6f", na_rep="NaN")
    genes.loc[genes.TargetGeneIsExpressed, :].to_csv(genelist_slim_expressed, sep="\t", index=False, header=True, float_format="%.6f", na_rep="NaN")
    genes_to_combine=genes.loc[genes.TargetGeneIsExpressed, ["TargetGene", "TargetGenePromoterActivityQuantile"]]
    genes_to_combine["Sample"]=args.sampleid
    genes_to_combine.to_csv(genelist_slim_expressed_to_combine, sep="\t", index=False, header=True, float_format="%.6f", na_rep="NaN" )
