...
def filter(args):
    pred_file_full = os.path.join(args.outdir, f"Predictions_ABC_threshold{args.alternative_threshold}", "EnhancerPredictionsFull.txt")
    pred_file_bedpe = os.path.join(args.outdir, f"Predictions_ABC_threshold{args.alternative_threshold}", "EnhancerPredictions.bedpe")
    all_putative_path = args.allPutative
    all_putative = pd.read_csv(all_putative_path, compression="gzip", sep="\t")
    all_positive = all_putative.iloc[np.logical_and.reduce((all_putative[args.score_column] > float(args.alternative_threshold), ~(all_putative['class'] == "promoter"))),:]
